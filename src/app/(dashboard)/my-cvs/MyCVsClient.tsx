...
import {
    ...
    Link as LinkIcon, AlertCircle, LayoutGrid, ChevronDown, FileEdit, Tag, Check, X, Loader2, Download, Mail
} from 'lucide-react';
...
function RenameModal({
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const res = await fetch(`/api/${doc.type === 'resume' ? 'resumes' : 'profiles'}/${doc.id}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ [doc.type === 'resume' ? 'job_title' : 'full_name']: trimmed }),
            });
...
function EditEmailModal({
    ...
    const [value, setValue] = useState(doc.email || '');
    const [saving, setSaving] = useState(false);
    const inputRef = useRef<HTMLInputElement>(null);
    ...
    const handleSave = async () => {
        const trimmed = value.trim();
        if (trimmed === (doc.email || '')) { onClose(); return; }
        setSaving(true);
        try {
            const res = await fetch(`/api/profiles/${doc.id}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email: trimmed }),
            });
            if (!res.ok) throw new Error('Failed to update email');
            onEmailUpdated(doc.id, trimmed);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-base font-extrabold text-zinc-900 dark:text-white">Edit Profile Email</h3>
                        <p className="text-xs text-zinc-500 dark:text-zinc-400">Used for Motivation Letters</p>
                    </div>
                </div>
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        onClick={handleSave}
                        disabled={saving || value.trim() === (doc.email || '')}
                        className="flex-1 flex items-center justify-center gap-1.5 py-2.5 px-4 rounded-xl text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-default transition-colors shadow-sm"
                    >
                        {saving ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Check className="w-3.5 h-3.5" />}
            ...
            {/* Reusing existing scaleIn animation from rename modal below or assuming it's globally defined by one of them */}
        </div>
    );
}
    ...
    return (
        <>
            <div ref={ref} className="flex-1 relative flex">
                <Link
                    href={editHref}
                    className={`flex-1 flex items-center justify-center gap-1.5 text-white text-sm font-bold py-2 pl-3 pr-2 rounded-l-lg transition-colors ${isProfile ? 'bg-blue-600 hover:bg-blue-700' : 'bg-indigo-600 hover:bg-indigo-700'}`}
                >
                    {isProfile ? <Pencil className="w-3.5 h-3.5" /> : <FileEdit className="w-3.5 h-3.5" />}
                    {isProfile ? 'Convert to CV' : 'Edit'}
                </Link>
                {/* Chevron */}
                <button
                    onClick={() => setOpen(o => !o)}
                    className={`flex items-center justify-center w-7 text-white border-l rounded-r-lg transition-colors ${isProfile ? 'bg-blue-600 hover:bg-blue-700 border-blue-500/60' : 'bg-indigo-600 hover:bg-indigo-700 border-indigo-500/60'}`}
                    title="More options"
                >
                ...
                {open && (
                    ...
                    >
                        ...
                        >
                            ...
                            {isProfile ? 'Edit Profile Content' : 'Edit CV Content'}
                        </Link>
                        ...
                        >
                            ...
                            {isProfile ? 'Rename Profile' : 'Rename CV'}
                        </button>
                        {isProfile && (
                            <>
                                <div className="h-px bg-zinc-100 dark:bg-white/5 mx-3" />
                                <button
                                    onClick={() => { setOpen(false); setEditingEmail(true); }}
                                    className="w-full flex items-center gap-2.5 px-3.5 py-2.5 text-sm font-semibold text-zinc-700 dark:text-zinc-200 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 hover:text-emerald-700 dark:hover:text-emerald-300 transition-colors"
                                >
