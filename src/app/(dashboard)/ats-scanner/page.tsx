...
import {
    ...
    CheckCheck, XCircle, Zap, ShieldCheck, Send,
} from "lucide-react";
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="h-2 w-full bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden mb-2">
                <div className="h-full rounded-full"
                    style={{ width: `${pct}%`, backgroundColor: sc.ring, transition: "width 0.8s ease-out" }} />
            </div>
...
export default function ATSScannerPage() {
    ...
    const resetAll = () => { setStep("upload"); setAtsResult(null); setCvText(""); setFileName(""); setFile(null); setJobDescription(""); setError(""); setImprovedCV(""); };
    ...
    const handleImprove = async () => {
        ...
        try {
            ...
            const data = await res.json();
            if (!res.ok) {
                if (data.code === "PRO_REQUIRED") { toast.error("ATS CV Improvement is a Pro feature. Please upgrade."); setStep("report"); return; }
                throw new Error(data.error || "Improvement failed");
            }
            setImprovedCV(data.improvedCV); setStep("improved");
    ...
    const handleDownload = () => {
        ...
        const a = document.createElement("a"); a.href = url; a.download = "ATS-Optimized-CV.txt"; a.click();
        ...
        return (
            <div className="min-h-screen bg-[#F9FAFB] dark:bg-zinc-950">
                ...
                <div className="max-w-3xl mx-auto p-6 md:p-12 space-y-6">
                    ...
                    {error && (
                        <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 text-red-700 dark:text-red-400 rounded-xl px-4 py-3 text-sm font-medium">{error}</div>
                    )}
                    ...
                    <button onClick={handleScan} disabled={step === "scanning" || (!cvText.trim() && !file)}
                        ...
                        {step === "scanning" ? (
                            ...
                            <><ScanLine className="w-5 h-5" /> Scan My CV</>
                        )}
                    </button>
                    ...
                    {step === "scanning" && (
                        <div className="flex flex-col items-center gap-3 py-8">
                            ...
                            <div className="flex gap-1.5 mt-1">
                                {[0, 1, 2].map(i => (
                                    <div key={i} className="w-2 h-2 rounded-full bg-indigo-400 dark:bg-indigo-500 animate-bounce" style={{ animationDelay: `${i * 0.15}s` }} />
                                ))}
                            </div>
                        </div>
                    )}
            ...
            <div className="min-h-screen bg-[#F9FAFB] dark:bg-zinc-950">
                ...
                <div className="bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-white/10 py-10 px-6">
                    <div className="max-w-4xl mx-auto text-center space-y-3">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-xs font-bold uppercase tracking-widest border border-indigo-100 dark:border-indigo-500/20">
                            <ScanLine className="w-3.5 h-3.5" /> ATS Analysis Complete
                        </div>
                        <h1 className="text-3xl md:text-4xl font-extrabold text-zinc-900 dark:text-white tracking-tight">Your ATS Score Report</h1>
                    </div>
                </div>
                ...
                <div className="max-w-4xl mx-auto p-6 md:p-12 space-y-10">
                    ...
                    {hasMissingKeywords && (
                        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 rounded-2xl p-6 shadow-sm space-y-4">
                            ...
                            <p className="text-xs text-zinc-500 dark:text-zinc-400">Keywords present in the job description but missing from your CV. Add these to increase match rate.</p>
                            <div className="flex flex-wrap gap-2">
                                {atsResult.missing_keywords.map((kw, i) => (
                                    <span key={i} className={`text-xs font-bold px-3 py-1.5 rounded-full ${KEYWORD_BADGE[kw.importance]}`}>
                                        {kw.keyword}
                                        <span className="ml-1.5 opacity-70 text-[10px] uppercase">{kw.importance === "nice-to-have" ? "nice" : kw.importance}</span>
                                    </span>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="space-y-3">
                        <button onClick={handleImprove} disabled={step === "improving"}
                            ...
                            {step === "improving" ? (
                                <><Loader2 className="w-5 h-5 animate-spin" /> Generating your ATS-optimized CV...</>
                            ) : (
                                <><Sparkles className="w-5 h-5" /> Improve My CV with AI <ArrowRight className="w-4 h-4" /></>
                            )}
                        </button>
                        <p className="text-center text-xs text-zinc-400 dark:text-zinc-600">Pro feature — uses AI to rewrite your CV fixing all issues above</p>
                    </div>

                    <div className="text-center">
                        <button onClick={resetAll} className="text-sm font-bold text-zinc-500 dark:text-zinc-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">← Scan Another CV</button>
                    </div>
                </div>
            </div>
        );
                ...
                <div className="bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-white/10 py-10 px-6">
                    <div className="max-w-4xl mx-auto text-center space-y-3">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-green-50 dark:bg-green-500/10 text-green-600 dark:text-green-400 text-xs font-bold uppercase tracking-widest border border-green-100 dark:border-green-500/20">
                            <CheckCircle2 className="w-3.5 h-3.5" /> Optimization Complete
                        </div>
                        <h1 className="text-3xl md:text-4xl font-extrabold text-zinc-900 dark:text-white tracking-tight">Your ATS-Optimized CV</h1>
                        <p className="text-zinc-500 dark:text-zinc-400 font-medium">All issues have been addressed. CAR method applied throughout.</p>
                    </div>
                </div>
