...
export async function POST(request: NextRequest) {
    try {
        ...
        // Start the run via native fetch with 1GB memory to prevent Actor OOM crashes, maximum allowed by actor per URL
        const response = await fetch(`https://api.apify.com/v2/acts/${ACTOR_ID}/runs?token=${token}&memory=1024`, {
            method: 'POST',
            headers: {
