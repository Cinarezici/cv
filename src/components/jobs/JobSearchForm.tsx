    ...
    return (
        <div className="w-full max-w-4xl mx-auto space-y-4">
            <form
                ...
                className="bg-white border border-zinc-200 rounded-3xl shadow-sm p-2 flex flex-col md:flex-row items-center gap-2"
            >
                {/* Keywords Input */}
                <div className="flex-1 flex items-center relative w-full border-b md:border-b-0 md:border-r border-zinc-100 pr-2 group">
                    <Search className="w-5 h-5 text-zinc-400 absolute left-4 group-focus-within:text-indigo-500 transition-colors" />
                    <input
                        type="text"
                        placeholder="Job title, keywords, or company"
                        className="w-full border-0 focus:ring-0 pl-12 h-12 text-sm md:text-base rounded-full bg-transparent outline-none"
                        value={keywords}
                    ...
                    {keywords && (
                        <button
                            type="button"
                            onClick={() => setKeywords('')}
                            className="p-1 hover:bg-zinc-100 rounded-full mr-2"
                        >
                ...
                <button
                    ...
                    className="w-full md:w-auto h-12 px-8 rounded-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 text-white font-bold transition-all shadow-md hover:shadow-lg active:scale-95 flex items-center justify-center gap-2 shrink-0"
                >
                    {isLoading ? (
                        <>
            ...
            <div className="flex flex-wrap items-center gap-2 px-4 justify-center md:justify-start">
                <span className="text-xs font-bold text-zinc-400 uppercase tracking-wider mr-1">Popular:</span>
