...
class AnimationController {
    ...
    public showProjectedDot(position: Vector3D, sizeFactor: number) {
        ...
        if (position.z > newCameraZ) {
            ...
            // USE CV OPTIMIZER BRAND BLUE COLOR: #2563eb or rgb(37, 99, 235)
            // Using rgba to allow transparency based on depth if needed, but solid works well
            this.ctx.fillStyle = 'rgba(37, 99, 235, 1)'
    ...
    public render() {
        ...
        // Background color changed to white/fafafa to match page styling
    ...
    private drawTrail(t1: number) {
        for (let i = 0; i < this.trailLength; i++) {
            ...
            // Trail color matched to brand blue #2563eb
...
export function SpiralAnimation({ play = true }: { play?: boolean }) {
    ...
    // 处理窗口大小变化 - Modified to match parent container rather than full window
    useEffect(() => {
        const handleResize = () => {
        ...
        // Also setup ResizeObserver for robust sizing
    ...
    useEffect(() => {
        ...
        // 使用实际尺寸，但因为是螺旋动画，最好取较大值或确保有足够绘制控件
